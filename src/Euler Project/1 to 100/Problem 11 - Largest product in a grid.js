...
function largestGridProduct(arr) {
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      if (j + 3 < cols) {
        const prod = arr[i][j] * arr[i][j + 1] * arr[i][j + 2] * arr[i][j + 3];
        if (prod > max) max = prod;
      }
      if (i + 3 < rows) {
        const prod = arr[i][j] * arr[i + 1][j] * arr[i + 2][j] * arr[i + 3][j];
        if (prod > max) max = prod;
      }
      if (i + 3 < rows && j + 3 < cols) {
        const prod = arr[i][j] * arr[i + 1][j + 1] * arr[i + 2][j + 2] * arr[i + 3][j + 3];
        if (prod > max) max = prod;
      }
      if (i + 3 < rows && j - 3 >= 0) {
        const prod = arr[i][j] * arr[i + 1][j - 1] * arr[i + 2][j - 2] * arr[i + 3][j - 3];
